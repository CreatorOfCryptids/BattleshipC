 ...
 *
 * @param map The map to be initialized as a 2D char array with each value set to ' '.
 */
...
/**
 * Places a ship into a map with the desired coordinates and orientation.
 *
...
/**
 * Gets the users target coordinate, sends it to the opponent process, and prints the opponent's response.
 *
...
void singleplayer(int input, int output)
{
    ...
    char cpu_hit_map[MAP_SIZE][MAP_SIZE];   // Map containing coordinates the CPU has attacked.
    ...
    for (int i = 0; i < SHIP_COUNT; i++)
    { // Skip first entry because already placed Carrier.
        do
        {
            if ((orientation = rand() % 2) == 1)
            { // Vertical
...
/**NOT YET IMPLEMENTED!
 * The code for the multiplayer opponent process that RECEIVES information from a client.
 *
 * @param input The pipe ID to send info TO the host Process FROM the child opponent process.
 * @param output The pipe ID to send info FROM the host process the TO child opponent process.
 */
...
int main()
{
    ...
    int inputs[2];  // Information coming FROM the chosen opponent
    int outputs[2]; // Information going TO the chosen opponent
    ...
    pid_t opponent_process = 0;

    do
    { // Print menu and get selections.
        print_menu();

        if ((choice = get_menu_selection()) == 1)
        { // If singleplayer, fork a process to play battleship with rand.
            // opponent_process = init_singleplayer(inputs[1], outputs[0]);
            if ((opponent_process = fork()) == 0)
            {
                singleplayer(inputs[1], outputs[0]);
            }
            else if (opponent_process == -1)
            {
        ...
        { // If multiplayer, ask if the user wants to host or connect.
    ...
    int op_hits = 0;     // Amount of hits AGAINST the opponent
    int player_hits = 0; // Amount of hits AGAINST the player
    ...
    {
        perror("HOST: Issue writing to output.");
        exit(1);
    }
    ...
    if (response == 'R')
    { // Capital R means that this user goes second.
        printf("You go second.\n\n");
    }
    else if (response == 'r')
    { // This user goes first.
    ...
    {
        perror("HOST: received incorrect init response.");
        exit(1);
    }

    // While both players have an un-sunk battleship, let them hit back and forth.
    while (op_hits < TOTAL_HITS - 1 && player_hits < TOTAL_HITS - 1)
    {
        ...
        if (target == 'C')
        {
            printf("Your opponent sunk your Carrier!!!\n\n");
        }
        else if (target == 'B')
        {
            printf("Your opponent sunk your Battleship!!!\n\n");
        }
        else if (target == 'D')
        {
            printf("Your opponent sunk your Destroyer!!!\n\n");
        }
        else if (target == 'S')
        {
            printf("Your opponent sunk your Submarine!!!\n\n");
        }
        else if (target == 'P')
        {
            printf("Your opponent sunk your Patrol Boat!!!\n\n");
        }
 ...
 * @param input The pipe IDs to send info TO the host Process FROM the child opponent process.
 * @param output The pipe IDs to send info FROM the host process the TO child opponent process.
 * @param connection_choice The user's decision to host (1) or connect (0);
*
